...
router.get('/check-completed', async (req, res) => {
    ...
    if (!rawProject) return res.status(400).json({ error: 'Missing required query parameter: project' });
    ...
    try {
        ...
        if (process.env.DB_HOST) {
            ...
            const [surveyRes, eventRes, scanRes] = await Promise.all([
                ...
                pool.query(`SELECT id FROM events WHERE project_name = $1 AND event_type = ANY($2) AND timestamp >= $3 LIMIT 1`,
                    [project, ['survey_submitted', 'declined_at_step_1'], oneDayAgo]),
                pool.query(`SELECT id FROM events WHERE project_name = $1 AND event_type = 'scanned' AND timestamp >= $2 LIMIT 1`,
            ...
            const [surveyRes, eventRes, scanRes] = await Promise.all([
                supabase.from('survey_results').select('id').eq('project_name', project).limit(1),
                supabase.from('events').select('id').eq('project_name', project)
                    .in('event_type', ['survey_submitted', 'declined_at_step_1'])
                    .gte('timestamp', oneDayAgo).limit(1),
                supabase.from('events').select('id').eq('project_name', project)
                    .eq('event_type', 'scanned').gte('timestamp', thirtyMinsAgo).limit(1)
            ]);
            return res.json({
                completed: !!(surveyRes.data && surveyRes.data.length > 0),
                final_step_done: !!(eventRes.data && eventRes.data.length > 0),
                scanned: !!(scanRes.data && scanRes.data.length > 0)
            });
        }
    } catch (err) {
...
router.post('/', async (req, res) => {
    console.log('[survey POST] body:', JSON.stringify(req.body));
        ...
        if (process.env.DB_HOST) {
            ...
            if (session_id) {
                const { rows } = await pool.query(
                    `SELECT id FROM events WHERE session_id = $1 AND event_type = 'survey_submitted' LIMIT 1`, [session_id]);
                if (rows.length > 0) return res.json({ id: session_id, message: 'คุณได้ส่งความคิดเห็นนี้เรียบร้อยแล้ว' });
            }
            if (project_name) {
                const { rows } = await pool.query('SELECT id FROM survey_results WHERE project_name = $1 LIMIT 1', [project_name]);
                if (rows.length > 0) return res.json({ id: 'duplicate', message: 'คุณได้ส่งความคิดเห็นนี้เรียบร้อยแล้ว' });
            }
            const { rows } = await pool.query(
            ...
            if (session_id) {
                const { data } = await supabase.from('events').select('id')
                    .eq('session_id', session_id).eq('event_type', 'survey_submitted').limit(1);
                if (data && data.length > 0) return res.json({ id: session_id, message: 'คุณได้ส่งความคิดเห็นนี้เรียบร้อยแล้ว' });
            }
            if (project_name) {
                const { data } = await supabase.from('survey_results').select('id').eq('project_name', project_name).limit(1);
                if (data && data.length > 0) return res.json({ id: 'duplicate', message: 'คุณได้ส่งความคิดเห็นนี้เรียบร้อยแล้ว' });
            }
            const id = randomUUID();
            const submitted_at = new Date().toISOString();
            await insertRow('survey_results', {
                id, submitted_at, session_id: session_id||'', employee_id: employee_id||'',
                employee_name: employee_name||'', project_name: project_name||'', customer_name: customer_name||'',
                pdpa_consent_1: pdpa_consent_1||'', score_q1, score_q2, score_q3, score_q4,
                improvements: improvements||'', improvements_other: improvements_other||'',
                contact_name: contact_name||'', contact_phone: contact_phone||'',
                contact_email: contact_email||'', pdpa_consent_2: pdpa_consent_2||''
            });
            return res.json({ id, submitted_at, message: 'ขอบคุณสำหรับความเห็น' });
        }
    } catch (err) {
