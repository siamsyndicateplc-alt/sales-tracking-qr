...
router.post('/', async (req, res) => {
    ...
    try {
        if (process.env.DB_HOST) {
            ...
            return res.json({ id: rows[0].id, timestamp: rows[0].timestamp, event_type, message: 'Event recorded' });
        } else {
            ...
            return res.json({ id, timestamp, event_type, message: 'Event recorded' });
        }
    } catch (err) {
